...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            int red = round(((float)image[i][j].rgbtBlue * .189 + (float)image[i][j].rgbtGreen * .769 + (float)image[i][j].rgbtRed * .393));
            int green = round(((float)image[i][j].rgbtBlue * .168 + (float)image[i][j].rgbtGreen * .686 + (float)image[i][j].rgbtRed * .349));
            int blue = round(((float)image[i][j].rgbtBlue * .131 + (float)image[i][j].rgbtGreen * .534 + (float)image[i][j].rgbtRed * .272));
